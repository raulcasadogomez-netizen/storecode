...
-- Habilitar Seguridad de Nivel de Fila (RLS) en customer_emails
ALTER TABLE public.customer_emails ENABLE ROW LEVEL SECURITY;

-- Políticas de RLS para customer_emails
DROP POLICY IF EXISTS "Permitir insercion publica de correos" ON public.customer_emails;
CREATE POLICY "Permitir insercion publica de correos" 
ON public.customer_emails 
FOR INSERT 
WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir lectura publica de correos" ON public.customer_emails;
CREATE POLICY "Permitir lectura publica de correos" 
ON public.customer_emails 
FOR SELECT 
USING (true);

DROP POLICY IF EXISTS "Permitir borrado de correos" ON public.customer_emails;
CREATE POLICY "Permitir borrado de correos" 
ON public.customer_emails 
FOR DELETE 
USING (true);
